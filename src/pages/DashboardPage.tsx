...
export default function DashboardPage() {
  ...
  const avgCheck =
    confirmedMonthBookings.length > 0
      ? Math.round(
          confirmedMonthBookings.reduce((sum, b) => {
            const svc = services.find((s) => s.id === b.service_id);
            return sum + (svc?.price || 0);
  ...
  const serviceStats = services
    .map((s) => ({
      ...
      revenue:
        bookings.filter((b) => b.service_id === s.id && b.status === 'confirmed').length *
        s.price,
    }))
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-gray-950 text-white h-screen sticky top-0">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {(
            ...
            ] as { id: Tab; icon: typeof Calendar; label: string }[]
          ).map((item) => (
            <button
              key={item.id}
          ...
          {/* ── Вкладка Админ Панели (только для ADMIN_UUID) ── */}
          {isAdmin && (
            <button
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-gray-950 border-t border-white/8 flex">
        ...
        {/* ── 5-я вкладка "Админка" для ADMIN_UUID ── */}
        {isAdmin && (
          <button
            onClick={() => navigate('/admin')}
            className="flex-1 flex flex-col items-center gap-1 py-3 text-xs font-medium transition-colors cursor-pointer text-red-400"
          >
            <Shield size={20} />
            Админка
          </button>
      ...
      <main className="flex-1 overflow-auto pb-20 lg:pb-0 relative">
        ...
        <div className="lg:hidden bg-gray-950 text-white px-4 py-3 flex items-center justify-between sticky top-0 z-20">
          ...
          {/* Центр: имя + статус подписки с днями */}
          ...
          <button
            onClick={handleLogout}
            className="text-gray-400 hover:text-white transition-colors cursor-pointer p-1 shrink-0"
          >
            <LogOut size={17} />
          </button>
        </div>
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() =>
                        setRevenueMonth(
                          new Date(revenueMonth.getFullYear(), revenueMonth.getMonth() - 1)
                        )
                      }
                    ...
                    <span className="text-xs text-gray-600 px-2">
                      {revenueMonth.toLocaleDateString('ru-RU', {
                              ...
                              {startingBookings.map((b) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <p className="opacity-70 truncate leading-tight text-xs">
                                      {b.service_name}
                                    </p>
                                    <p className="opacity-50 leading-tight text-xs">{b.time}</p>
                                    {slotsCount > 1 && (
                                      <p className="opacity-40 text-xs leading-tight">
                                        {duration} мин
                                      </p>
                                    )}
                                  </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    <div className="p-4 border-b border-gray-100 bg-emerald-50">
                      ...
                      <p className="text-sm text-gray-500">
                        {getDayBookings(selectedDay).length} записей
                      </p>
                    </div>
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <p className="opacity-70">
                                        {b.service_name} · {b.time} · {duration} мин
                                      </p>
                                    </div>
                                  );
                                })
                ...
                <div className="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-amber-700 mt-0.5">
                      До {FREE_SERVICES_LIMIT} услуг на бесплатном тарифе. Перейдите на PRO.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  <div className="bg-white rounded-2xl border border-gray-200 p-5">
                    ...
                    <p className="text-3xl font-black text-gray-900">
                      {calculateMonthRevenue(new Date()).toLocaleString('ru-RU')} ₽
                    </p>
                                ...
                                style={{
                                  width:
                                    serviceStats[0].count > 0
                                      ? `${(s.count / serviceStats[0].count) * 100}%`
                                      : '0%',
                                }}
                              />
                            </div>
            ...
            <div className="max-w-2xl space-y-6">
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-sm text-emerald-600 hover:text-emerald-700 transition-colors mt-3"
                >
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <p className="text-sm text-gray-500 mb-5">
                  Получайте уведомления о новых записях в Telegram.
                </p>
                <a
                ...
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl mb-5">
                  <p className="text-xs text-blue-700 leading-relaxed">
                    ...
                    1. Нажмите кнопку выше — откроется Telegram<br />
                    2. Нажмите <strong>/start</strong> в боте<br />
                    3. Бот автоматически привяжет аккаунт<br />
                    4. Готово! Получайте уведомления о каждой записи
                  </p>
                </div>
                ...
                <details className="group">
                  ...
                  <div className="mt-4 space-y-3">
                    <p className="text-xs text-gray-500">
                      Узнать Chat ID:{' '}
                      <span className="font-mono text-emerald-600">@userinfobot</span>
                    </p>
                    <Input
